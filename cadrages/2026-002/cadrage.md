...
les renommages. La vérification d'intégrité doit signaler la disparition d'un
identifiant.
...
Les champs `cree_par` et `modifie_par` sont un index de performance, écrit par la
propagation automatique et jamais à la main : la source de vérité reste le log
Git.

### RG-enonces-dans-cadrage

Un impact de type `cree` ou `modifie` doit être accompagné du **nouvel énoncé** de
la règle, dans une section `## Énoncés` du cadrage, sous un titre de niveau 3
portant l'identifiant de la règle.
...
référentiel : la vérification ne l'exige qu'à la livraison.

### RG-pj-adressage-contenu

Une pièce jointe est stockée sous le **hash SHA-256 de son contenu**, à la clé
`sha256/{ab}/{cd}/{hash}`. Le fichier versionné ne porte que ce hash, sa taille,
son nom d'origine et son type.
...
L'application installée ayant les mêmes droits quel que soit l'utilisateur, un
relais qui se contenterait de transmettre laisserait tout porteur d'une identité
valide écrire dans n'importe quel projet.
...
La recherche s'appuie sur un **index construit côté client**, invalidé par le SHA
de chaque fichier.

L'API de recherche de la plateforme n'est pas utilisable : dix requêtes par
minute, branche par défaut uniquement, aucune recherche historique, aucune
...
La reconstitution ne coûte que deux appels, quelle que soit la taille du
référentiel.
...
règle est-elle ainsi ? » devient une requête.
...
L'application lit l'état réel de la PR au lieu de maintenir un statut en
parallèle, ce qui rend la désynchronisation impossible.
...
commit suivant porte l'écriture faite par la machine.
